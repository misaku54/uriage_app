...
RSpec.describe AggregatesHelper, type: :helper do
  ...
  describe 'make_title' do
    context '引数がmaker_producttypeの場合' do
      let(:title) { 'メーカー×商品分類' }
      it '正しい文字列が返ること' do
  ...
  describe 'best_selling_name' do
    ...
    context '引数がメーカー×商品分類のオブジェクトの場合' do
      let(:name) { 'テスト会社のカバン' }
      it '正しい文字列が返ること' do
    ...
    context '引数が条件外のオブジェクトの場合' do
      it 'raiseすること' do
        expect{best_selling_name("foo")}.to raise_error(ArgumentError,"無効な引数が渡されました。aggregate: foo")
      end
    end
  end
end
